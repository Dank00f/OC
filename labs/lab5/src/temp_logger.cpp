...
static void gmtime_r_portable(const time_t* tt, std::tm* out) {
#ifdef _WIN32
    gmtime_s(out, tt);
#else
    gmtime_r(tt, out);
...
int main(int argc, char** argv) {
    ...
    string f_hour = logDir + "/hourly_avg.log";
    string f_day  = logDir + "/daily_avg.log";
    ...
    auto roll_hour = [&](time_t hstart) {
        ...
        hourly << iso_utc_from(hstart) << "," << fixed << setprecision(3) << a << "\n";
        hourly.flush();
        acc_h.reset();
    };
    ...
    auto roll_day = [&](time_t dstart) {
        ...
        double a = acc_d.avg();
        if (!std::isfinite(a)) return;
        daily << iso_utc_from(dstart) << "," << fixed << setprecision(3) << a << "\n";
        daily.flush();
        acc_d.reset();
    };
    ...
    if (simulate) {
        ...
        while (!g_stop && getline(cin, line)) {
            ...
            double v = 0.0;
            try { v = stod(line.substr(p + 1)); } catch (...) { continue; }
            feed_sample(ts, v);
        }
    }
