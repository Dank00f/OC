             ...
             Process*& out_proc)
{
    ...
    std::string full = q(program);
    for (const auto& a: args){ full.push_back(' '); full += q(a); }
    ...
    if (!okc){ auto s=sys_error("CreateProcess", GetLastError()); delete p; return s; }
    p->started = true;
#else
    pid_t pid = fork();
...
Status wait(Process* p, millis timeout_ms){
    ...
    DWORD wr = WaitForSingleObject(p->pi.hProcess, t);
...
Status exit_code(const Process* p, int& out_code){
    ...
    if (WIFSIGNALED(status)){ out_code=128+WTERMSIG(status); return ok(); }
