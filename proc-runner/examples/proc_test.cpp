...
int main(int argc, char** argv){
    ...
    SpawnOptions opts; int64_t timeout=-1; std::vector<std::string> argsv; std::string program=argv[1];
    for(int i=2;i<argc;++i){
        std::string a=argv[i];
        if(a=="--timeout-ms" && i+1<argc)      timeout=std::stoll(argv[++i]);
        else if(a=="--workdir" && i+1<argc)    opts.workdir=argv[++i];
        else                                    argsv.push_back(a);
    }
    proc::Process* p=nullptr; auto s=spawn(program,argsv,opts,p);
    if(s.code!=Result::Ok){ print_status(s); return 1; }
    std::cout<<"spawned, running="<<(is_running(p)?"yes":"no")<<"\n";
    s=wait(p,timeout); print_status(s);
    if(s.code==Result::Timeout){ std::cout<<"still running, waiting...\n"; s=wait(p,-1); print_status(s); }
    int code=-1; s=exit_code(p,code);
